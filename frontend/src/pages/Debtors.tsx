...
function UnallocatedTab() {
  ...
  const warns = (
    <>
      {/* Запрос упал целиком: список пуст не потому, что счетов нет. */}
      {data?.error && (
        <div style={{ padding: "8px 28px", background: "#FBF3F2", borderBottom: "1px solid #EDEBE6", fontSize: 11, color: "#8B3A3A" }}>
          ⚠ Счета не загрузились: {data.error} — список пуст из-за ошибки, а не потому, что счетов нет.
        </div>
      )}
