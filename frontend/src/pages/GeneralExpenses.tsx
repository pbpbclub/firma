...
const PURPOSES = [
  ...
  // Контур лицевого счёта подрядчика: заводятся в его карточке, но живут здесь же —
  // это тоже траты без заказа. В фильтре их не было, и при «Все» они шли безымянно.
  { v: "contractor_pay", l: "Выплата подрядчику",
    hint: "Деньги подрядчику вне заказа за уже сделанное (лицевой счёт)" },
  { v: "contractor_advance", l: "Аванс подрядчику",
    hint: "Деньги вперёд под будущую работу — только такая выплата даёт «аванс у мастера»" },
  { v: "contractor_third_party", l: "Оплата за подрядчика",
    hint: "Заплатили третьему лицу по его просьбе: деньги наши, себестоимость не наша" },
  // Изъятие прибыли, а не трата дела: в себестоимость, накладные и P&L общего
  // контура не входит — только сводка «сколько взято» (ТЗ 03.09.2026).
  { v: "owner_draw", l: "Личные средства владельца",
    hint: "Вывод себе: Avosend, перевод себе, снятие. Не расход бизнеса" },
];
const CATEGORIES = [
...
export default function GeneralExpenses() {
  ...
  const metrics = [
    ...
    { label: "ВЫВЕДЕНО СЕБЕ", value: sum?.owner_draw ?? 0, color: "#A89070" },
  ];
