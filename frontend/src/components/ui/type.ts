...
/**
 * Цвет суммы ДОЛГА. side="in" — нам должны, "out" — мы должны.
 * Ноль — нейтральный: долга нет, красить нечем и тревожить незачем.
 *
 * Правило Юры (24.08.2026): всё, что нам должны, — зелёное; всё, что должны мы, —
 * красное. До этого «клиент нам должен» красился ТРЕМЯ разными цветами: зелёным в
 * «Обязательствах», оранжевым в дашборде и карточке заказа, красным в карточке клиента.
 * Хуже: в лестнице прибыли, полосе-сводке и карточке клиента логика была перевёрнута —
 * «долг есть» тревожно-оранжевым, «долга нет» зелёным. Долг клиента это наши будущие
 * деньги: он зелёный, а ноль — просто ноль.
 *
 * Только для ДОЛГОВ. Прибыль/убыток, перерасход себестоимости и направление транзакции
 * в ДДС красятся своими правилами — там цвет значит не «кто кому должен».
 */
export function debtColor(amount: number | null | undefined, side: "in" | "out"): string {
  if (!amount) return POLARITY.neutral.color;
  return (side === "in" ? POLARITY.in : POLARITY.out).color;
}
