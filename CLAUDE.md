...
### 🧮 Проектные работы: машинное время агентов (11.09.2026)

Второй вид деятельности — чертежи и модели (Профи.ру, бренд pbpb): себестоимость =
сессии агента (конструктор на маке Юры; будут другие люди/агенты/площадки).
Решения Юры 09.09 и 11.09.2026: **деньги считает токен** (тариф модели × курс,
снимок на момент записи), **час — вторая координата**, ставки часа нет; **единая
таблица Фирмы `machine_usage`, приём только через API**; часы Юры пока не вносятся.
- Роутер `routers/machine_usage.py`: `POST /api/machine-usage` (одна запись),
  `POST …/import` (`{entries:[…]}` — формат `mac_<дата>.json` фин-агента как есть,
  `in`/`out` как синонимы, `note: "папка X"` → `project_dir`), `GET …?order_id|month|
  unassigned=1`, `PATCH …/{id}` (назначить заказ → заводится расход), `DELETE …/{id}`
  (с расходом; 409, если расход закрывает обязательство), `GET …/summary` (по
  заказам: цена из `_margin`, `machine_cost`, `executor_paid` = расходы work/other без
  строки machine_usage, часы, токены, `rub_per_hour`, `rub_per_mtok`, `cost_share`,
  `tok_per_hour`; `by_agent`; `unassigned`), `GET/PUT …/models` (тарифы `model_prices`,
  $ за 1M по четырём видам), `GET/PUT …/settings` (`app_settings.usd_rate`).
- 🔒 **Неизвестная модель — 400 с перечнем известных**, не расчёт «по похожей»
  (fable-5-1 впятеро дороже sonnet, тихое занижение хуже отсутствия).
- Расход по заказу — через `orders._insert_expense` (та же дверь, что карточка:
  автопривязка обязательства, касса, аудит); `machine_usage.expense_id` — связь.
  `expense_id` в запросе = перенос уже заведённого расхода: рубли берутся из него,
  курс — производный, второго расхода нет. `_plan_fact` про токены не знает.
- Дедуп — `ext_key` (ключ источника `mac-<дата>-<папка>-<модель>` или sha1 строки):
  повтор → `status: exists`.
- `orders.activity` (`production` дефолт | `design`) — в списке, карточке, форме
  создания и «Параметрах»; `order_project_dirs(dir PK, order_id)` — папка
  конструктора → заказ (ТЗ Mac), `GET /api/orders/project-dirs` → плоский словарь
  для мака, `PUT /orders/{id}/project-dirs {dirs}` (409 `dir_taken`). Seed в миграции
  `ensure_order_activity_schema` (`INSERT OR IGNORE`).
- Страница `/design` (`pages/DesignWork.tsx`, пункт «Проектные» в nav): метрики
  направления, таблица по заказам с раскрытием сессий, блок «Без заказа» (назначить
  + запомнить папку), разрез по агентам, тарифы и курс. Карточка заказа — блок
  «Машинное время» (токены заголовком, ₽ рядом, папки).
- Передачи 11.09.2026: fin — `design_costs.py` → `POST …/import`, ORD-053/054 с
  `expense_id`; mac — карта из `GET /orders/project-dirs`; yos — `production.py`
  (`order --project-dir`, `project-dirs --json`, `expense-add --master`).
