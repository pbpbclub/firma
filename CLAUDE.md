...
### 👁 Разноска видна в лентах, история заказа (25.08.2026)

- **Подпись «куда разнесено» — один компонент `components/money/AllocNote.tsx`**
  на ДДС и личные финансы: заказ ссылкой + комментарий расхода + «откатить» по
  `group_id`. Данные — `GET /api/expenses/map` (ключи `bank:` / `zen:`). Личные
  финансы фильтруют `zen:`, ДДС — `bank:`; НЕ показывать zen-разноску в ДДС —
  этих транзакций в банковской ленте нет физически.
