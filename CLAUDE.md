...
- **История заказа** — `GET /api/orders/{id}/timeline` из `audit_log` (сущности
  собираются к заказу джойнами, удалённые — `json_extract(changes,'$.order_id')`).
...
- `ordersApi.list` всегда шлёт `limit=200` (потолок бэка `le=200`) — иначе сервер
  отдаёт 50 и справочники выбора заказа молча обрезаются. Вызовы через обёртку,
  свой limit не писать.
