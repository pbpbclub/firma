...
✅ **WAL и права — починено 29.07.2026, обходные пути больше не нужны.**
Раньше `*-wal` / `*-shm` создавал тот процесс, который открыл базу (сервис под **root**),
ACL для `claude-runner` на сайдкары не наследовались, и база из сессии агента открывалась
через раз: «unable to open database file» при живом приложении. Кто последним пересоздал
журнал — тот им и владел.

Лечится не ACL, а **группой**: SQLite копирует на сайдкары режим и группу основного файла,
поэтому общие базы (`production`, `wiki`, `finance`, `zenmoney`, `analytics`) переведены в
группу `claude-runner` с режимом 660. Журналы наследуют её автоматически, кто бы базу ни
открыл. Проверено при живом root-овом WAL.

- Прямые запросы к БД из сессии теперь работают стабильно — читай через
  `file:...?mode=ro` (чистый читатель, иначе ты случайный писатель).
- `db.get_analytics_ro()` с фолбэком на `immutable=1` остаётся, но как страховка:
  он читает мимо WAL и свежих незачекпойнченных записей не увидит.
- **Изменения всё равно проверяй через HTTP API с JWT.** Причина уже не в правах, а в том,
  что приложение видит данные через свою логику: прямой SELECT покажет строку, а эндпоинт —
  то, что реально уедет на фронт.
...
🔒 **Секреты живут только в `/opt/firma/backend/.env` — синк бэкенда их не трогает.**
29.07.2026 `cp -r backend/.` (точка тащит и dotfiles) затёр боевой `.env` локальным
из `.gitignore` — без `FIRMA_SECRET_KEY` `auth.py` падает на старте, сервис ушёл в
рестарт-луп. Теперь синк идёт `find … ! -name '.env*'`, а перед рестартом deploy.sh
проверяет четыре ключа в боевом `.env` и отказывается рестартовать, если их нет.
**Копировать бэкенд «целиком» больше нельзя** — ни `cp -r backend/.`, ни `rsync` без
`--exclude .env*`.
...
**Одна тема — одно сообщение.** У smart-сессии получателя потолок 90 ходов и 30 минут (поднят
30.07.2026 с 40/15). Сводка из пяти пунктов (схема + формулы + статус + поля API + просьба)
29.07.2026 упёрлась в лимит на четвёртом пункте: фин успел починить три вещи, четвёртую не
начал, а поручение закрылось ошибкой. Накопились изменения — шли их отдельными поручениями,
каждое со своим предметом.

Дело не только в обрывах. Замер 30.07.2026: **цена сессии растёт квадратично по числу ходов**,
потому что каждый ход перечитывает весь накопленный контекст (в измеренной сессии он рос
39K → 109K токенов за 98 ходов). **40 ходов ≈ $1.2, 90 ходов ≈ $5-8.** Три коротких поручения
по 30 ходов стоят ~$2 вместо $6 за одно длинное — и ни одно не рискует оборваться на середине.
Холодный старт (CLAUDE.md + инструменты) при этом всего $0.16, 7% цены: экономить на объёме
инструкций бессмысленно, экономить надо на длине сессии.

Отсюда, когда **выполняешь** входящее поручение:
- не читай файлы целиком — `Read` с `offset`/`limit`, `grep -n` вместо `cat`;
- не печатай длинные диффы и дампы: они остаются в контексте до конца сессии и оплачиваются
  каждым следующим ходом;
- закончил свою тему — отчитайся и заверши, не ищи смежные проблемы по дороге;
- **упёрся в потолок ходов — напиши в отчёте, что успел и с какого места продолжать.** Тогда
  следующий заход будет добиванием, а не повтором всей работы с нуля по полной цене.

Суточный бюджет канала — $25 (`/opt/ai-os/tools/inbox_budget.py`, ведёт YOS). Исчерпан →
smart-поручения ждут в статусе `deferred` и разберутся после продления. Ничего не теряется.

Проваленное поручение теперь висит в статусе `failed`, а не закрывается молча —
`agent_msg.py list --status failed`. Автоповтора нет: решает человек.
...
Твои отчёты по поручениям доходят до Юры **ботом YOS в Telegram** — своего бота у тебя нет.
Значит формат тот же, что у всех агентов среды.

По-русски, кратко: что сделано и результат. Ошибка — причина и следующий шаг. Заказ
называть по имени из `orders.title` («MIRRA»), а не кодом `ORD-XXX`.

**Без markdown-таблиц** (правило Юры 30.07.2026). В Telegram таблицы разъезжаются: ответ
уходит plain text, и `|---|---|` приезжает кашей. Данные списком — строка это абзац, первая
колонка заголовком, остальное под ней видом «поле: значение»:
...
Из двух колонок — плоско: `Выручка: 568 248 ₽`. Транспорт разворачивает таблицы сам
(`detable` в `tgsend.py` у YOS), но это страховка от забывчивости, а не разрешение:
развёрнутая машиной таблица читается хуже написанного списка. В интерфейсе самой Фирмы
таблицы, разумеется, остаются — правило только про текст в Telegram.
