...
- **Вывод себе за границу — по МАРШРУТУ** (`backend/abroad_routes.py`, 23.09.2026). До
  этого считались только кросс-строки, и сентябрь показывал 1 500 ₽ вместо ~156 тыс.:
  Avosend и почти вся Золотая корона лежат ОДНОЙ ногой — расходом с рублёвой карты.
  Маршруты: Avosend, Золотая корона (+KoronaPay), Узбекистан (MS 9, «себе» — решение
  Юры), прямой BOG, прочие кросс-строки. 🔒 Считается нога с ДОМАШНЕГО счёта наружу;
  «Т-Банк → Райффайзен» — перевод между своими, «Прочие поступления AVOSEND» — возврат.
  🔒 Это НЕ расход (решение Юры 23.09.2026): `zenmoney.report/cashflow` считают переводом,
  Разноска не показывает, `personal-spending.abroad` — по маршрутам. Бухгалтеру —
  «Себе за границу» без маршрута, получателя и назначения. `route_of` зовётся со
  служебной линзой (`scope_for(None, owner=True)`): классификация от доступа не зависит.
- **Вкладка «Пополнения»** (`GET /regions/{code}/topups`): слева ушло с рублёвых карт по
  маршрутам, справа пришло на карту страны (`abroad.topup_source`: с рублёвых карт /
  от людей / без отправителя; обмен внутри карты не пополнение). 🔒 Стороны по сумме НЕ
  стыкуются — ушло рублями, пришло лари по курсу сервиса; пара — отдельный шаг.
- **Окно расшифровки** — любая цифра аналитики открывает операции, которые её дали, с
  ТЕМИ ЖЕ фильтрами (`/transactions` + `payee_key`, `currency`, `weekday`, `date_exact`,
  `bucket_from/to`); там же правка категории. Пресет «Этот месяц» — календарный: Юра
  спросил про 443 ₾ «в этом месяце», а экран считал скользящие 30 дней (за сентябрь — 160).
...
### 📈 «Неделя» на главной = недельный отчёт фин-агента (25.09.2026)

`GET /api/reports/week` + `components/dashboard/WeekPanel.tsx` — блоки `weekly_report.py
--news` числами Фирмы: деньги недели (пн..сегодня, засечка — прошлая неделя той же длины),
приход р/с по 12 неделям, направления со спидометром маржи (`Gauge`), план/факт
себестоимости (`ui/BudgetBar`), «Молчат» с делениями 14/30/60 (`ui/TickBar`, данные
`/orders/silent`), доход по кварталам (`ui/Columns`). Правила сверены с отчётом до рубля:
«Пришло» = платежи по дате + приход выписки без платежа той же суммы (±1 ₽);
«Потрачено по проектам» = расходы без `owner_draw` + выплаты мастерам с личных карт вне
расходов (`master_ledger`/`zm_links`/`payee_rules` master, только рублёвая нога).
🔒 «Себе в Тбилиси» — по списку фин-агента `/opt/fin-agent/data/zm_tbilisi.json`
(решение Юры 25.09.2026: «финагент знает, как я вывожу»; своего справочника нет, файла
нет → фолбэк `abroad_routes`, `money.abroad_source`). Раздел «Грузия» считает по
маршрутам (`abroad_routes`) — это другой вопрос, его не трогать: прямой перевод на BOG
с 08.2026 лежит двумя строками («IURII N.» рублями + «shps niu pheiment sistem» лари),
маршрут рублёвую ногу не узнаёт, список — узнаёт.
**Шапка главной** — `components/dashboard/HeroPanel.tsx`: свободные деньги = р/с ИП + личные
счета домашнего контура (`/zenmoney/accounts-summary`, RUB, без Грузии и наличных) − резервы
− фонды (решение Юры 25.09.2026; резервы/фонды вычитаются сначала из р/с), полосой `ui/SegBar`;
под ней **«Баланс»** = свободные + дебиторка (`debtors.total`) − осталось потратить по
заказам в работе (`creditors.plan_rest_total`); 🔒 сальдо `ledger.we_owe` туда НЕ
прибавлять — начисления строятся из тех же строк обязательств, будет двойной счёт;
плитка «На счетах» — те же деньги по банкам, ДДС по месяцам парными
столбиками, «Пульс» — три `Gauge animate` (собрано по производству, квартал до срока УСН,
расходы к приходу месяца), плитки со своей инфографикой. Запросы главной — `refetchInterval`
5 мин (монитор), в шапке «обновлено ЧЧ:ММ».
**«Этот месяц» на главной** — `GET /api/reports/money-months`, деньги ДЕЛА правилами недельного
отчёта фин-агента (решение Юры 25.09.2026, теми же функциями, что «Деньги за неделю»):
поступило = платежи по дате + неразнесённый приход выписки (`_income_week`); потрачено =
расходы Фирмы без `owner_draw` + выплаты мастерам с личных карт (`_spent_projects` +
`_card_payouts`); **выведено себе** = с р/с на личный (`OWN_TRANSFER_SQL` out) — отдельно, НЕ
расход. 🔒 Бытовые траты с личных карт в «потрачено» не идут (первая версия «ИП + личные»
считала их — выводы себе читались расходом); Тбилиси к выводам не прибавлять — те же деньги.
**Низ главной** — `components/dashboard/LowerPanel.tsx`: «Что делать» карточками, производство
(оплата — `SegBar`, себестоимость — `BudgetBar`), бренды (доход = расходы серым + прибыль
цветом бренда; `GET /finance/by-brand?split_design=1` — проектные работы отдельной строкой,
а не внутри pbpb; «Без бренда» на главной скрыт — решение Юры 25.09.2026; без параметра
ручка прежняя — её читает вики брендов), «Нам должны» / «Мы должны» рядом с полосами по людям, накладные сегментами.
Главная целиком собрана из `HeroPanel` + `WeekPanel` + `LowerPanel`; свой `ThinBar` удалён.
Выборка заказов — `in_production` + `completed` с `updated_at` в неделе, цифры `_plan_fact`.
