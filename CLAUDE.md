...
**«Этот месяц» на главной** — `GET /api/reports/money-months` (решение Юры 25.09.2026): р/с ИП
+ личные счета одним контуром, без Грузии. Не оборот: переводы между своими (правило ДДС,
двуногие строки ZM, `payee_rules` self), чужие деньги, вывод за границу (список фин-агента +
`abroad_routes`), «не бизнес» по разметке фин-агента `/opt/fin-agent/data/zm_ignore.json`
(по `tx` или получателю: подарки, личные переводы, трейдинг, чужие деньги), и приход на личный, которому нашёлся перевод «себе» с р/с той же суммы
за ≤3 дня («Прочие поступления» Сбера без получателя). По одному р/с «выбыло» было
17 тыс. при реальных ~370: с р/с деньги уходят себе на карту, тратятся с карт.
...
цветом бренда), «Нам должны» / «Мы должны» рядом с полосами по людям, накладные сегментами.
