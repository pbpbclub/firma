...
**Одна тема — одно сообщение.** У smart-сессии получателя потолок 40 ходов и 15 минут. Сводка
из пяти пунктов (схема + формулы + статус + поля API + просьба) 29.07.2026 упёрлась в лимит на
четвёртом пункте: фин успел починить три вещи, четвёртую не начал, а поручение закрылось
ошибкой. Накопились изменения — шли их отдельными поручениями, каждое со своим предметом.
Так и по токенам дешевле: короткая сессия не тащит контекст пяти задач ради одной.
