...
def list_work_rates():
    ...
    try:
        ...
        return {"rates": [dict(r) for r in rows], "work_types_without_rate": [dict(r) for r in no_rate]}
    finally:
        conn.close()
...
def create_work_rate(body: WorkRateIn):
    ...
    try:
        ...
        return {**dict(row), "work_type_name": wt["name"], "master_name": master["name"] if master else None}
    finally:
        conn.close()
