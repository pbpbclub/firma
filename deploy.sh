...
# ронял: под `set -e` ложное условие слева от `&&` — не последняя команда списка,
# и оболочка его failure игнорирует (проверено на bash 5.2).
WAIT_SECONDS=40
...
while :; do
  ...
  # Пауза между попытками входит в БЮДЖЕТ: остаток пересчитываем ПОСЛЕ попытки и
  # ДО сна. Прежний безусловный `sleep 1` в конце итерации исполнялся уже за
  # дедлайном, и фактический потолок был 41 с при обещанных человеку 40 — тот же
  # разрыв «объявленный срок ≠ факт», ради которого цикл и переводили на бюджет
  # (code_rules 07.09.2026). Спим только когда до дедлайна остаётся не меньше
  # секунды, иначе выходим сразу.
  left=$(( deadline - $(date +%s) ))
  if [ "$left" -le 0 ]; then
    break
  fi
...
if [ "$ready" != "1" ]; then
  ...
  echo "!!! Фирма не отвечает через ${WAIT_SECONDS} с после рестарта — smoke не запускался." >&2
  python3 /opt/ai-os/tools/agent_msg.py send --to yos --type alert \
    --text "[инженер Фирмы] после рестарта Фирма не отвечает ${WAIT_SECONDS} с — smoke не запускался, сервис может лежать. Смотри вывод deploy.sh." || true
  exit 1
fi
