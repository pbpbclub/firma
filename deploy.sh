...
while :; do
  ...
  # Готовность — СОВПАДЕНИЕ с ожидаемым видом ответа (ровно три цифры и не 000),
  # а не «отличие от сентинела». Прежнее `[ "$code" != "000" ]` принимало за ответ
  # ЛЮБОЕ молчание curl: нет бинаря, процесс убит OOM, ошибка до записи
  # %{http_code} — stdout пуст, `"" != "000"` истинно, ожидание снималось досрочно
  # и smoke уходил по лежащему сервису без алерта (code_rules 08.09.2026).
  case "$code" in
    ''|000|*[!0-9]*) ;;        # пусто, сентинел или мусор — сервер не ответил
    ???) ready=1; break ;;     # любой настоящий HTTP-код: порт слушает
    *) ;;                      # не три цифры — тоже не ответ
  esac
